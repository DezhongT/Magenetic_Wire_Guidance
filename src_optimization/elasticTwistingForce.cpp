...
void elasticTwistingForce::computeJt()
{
    for (int i=1; i<rod->ne; i++)
    {
        ...
        D2mDe2 = -0.25 / norm2_e * (kbLocal * (te+tilde_t).adjoint()
            + (te+tilde_t) * kbLocal.adjoint());
        D2mDf2 = -0.25 / norm2_f * (kbLocal * (tf+tilde_t).adjoint()
            + (tf+tilde_t) * kbLocal.adjoint());
        D2mDeDf = 0.5  / (norm_e*norm_f) * (2.0 / chi * teMatrix
            - kbLocal*tilde_t.adjoint());
        D2mDfDe = D2mDeDf.adjoint();
        ...
        Jtt = GJ * milen * ((deltam(i)+rod->refTwist(i) - rod->undeformedTwist(i)) 
            * DDtwist + gradTwistLocal * gradTwistLocal.adjoint());
